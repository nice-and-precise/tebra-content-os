...
| 2026-04-21 | **`cache_control` TTL pinning is API-level, not `settings.json` configuration.** M5 spec required "explicit 1h cache_control TTL pinning for the brand-voice block" in `settings.json`. This does not map to any Claude Code `settings.json` key — prompt cache control is a parameter on the Anthropic API request, not a Claude Code project setting. The TTL pinning decision (Section 2.2 entry: "Cache TTL pinned `"ttl": "1h"` explicitly") applies to subagent code that makes direct API calls; it is not enforceable via `settings.json`. | Investigated at M6 when writing `tebra-brand-voice`. `settings.json` has no `cache_control` or `ttl` field in the Claude Code settings spec as of April 2026. The spec requirement was forward-looking and unfulfillable at M5 without a skills-aware runtime hook. Gap documented; no action required for v1. | Active gap — v1.1 target if Claude Code exposes skills-level cache hints |
...
| 2026-04-21 | **`quick_answer` and `refresh` asset types have no dedicated skill.** `quick_answer` content is served by the `citation-block-library` skill's `references/quick-answer.md` template (it is a block type, not a full-page asset type). `refresh` is a workflow managed by the `refresh-auditor` subagent — it audits existing pages for staleness, it does not produce a new content type. Neither needs a standalone skill file. | Both were considered during M6. A standalone `bofu-quick-answer` skill would duplicate what `citation-block-library` already teaches. A `bofu-refresh` skill would duplicate `refresh-auditor.md`. The correct primitives for those two cases are already in place. | Active — intentional gap, not an oversight |
...
| 2026-04-21 | **`validate_sources.py` checks schema + file presence + expiry only — not claim-type alignment with drafts.** The validator confirms each source record is Pydantic-valid, its `path` resolves on disk, and `expires_at` is not past. It does not cross-check which claim types each source is cited for inside existing draft files. | Claim-type alignment at draft time is the compliance hook's job (`hooks/pre-commit/compliance-check.py`). Running that check in `validate_sources.py` would require loading every draft, creating a dependency on draft schema and making the validator stateful across the whole repo. The narrow scope keeps the CI step fast and deterministic. | Active — intentional scope bound |
...
| 2026-04-21 | **`async: true` hook flag is not in Claude Code's hook schema.** TASKS.md M8 spec references `async: true` as a settings.json hook property for Stop and PostToolUse hooks. Verified against the actual Claude Code settings.json hook schema (April 2026): hook objects accept `type`, `command`, `timeout`, and `statusMessage` only — no `async` field. | Implemented async behavior via `command &` backgrounding inside the hook scripts themselves (`stop-run-linters.sh`, `post-commit-changelog.sh`). The shell subshell `( ... ) &` and `exit 0` immediately returns control to Claude Code; the linters/changelog work runs in the background. Functionally equivalent to `async: true` without needing schema support. If Claude Code adds an `async` flag in a future release, it can replace `command &` with a settings.json property. | Active; revisit if Claude Code adds async hook flag |
| 2026-04-21 | **compliance-qa subagent is standalone — hooks cannot invoke Claude subagents.** TASKS.md M8 spec says compliance-qa is "called by the PreToolUse hook for nuanced cases." PreToolUse hooks are shell scripts that read stdin JSON and write stdout JSON. A hook cannot invoke a Claude subagent — it can only exec a shell command. Shell processes and Claude agent sessions are different execution contexts. | compliance-qa is implemented as a standalone subagent that draft-writer invokes directly (not via the hook). The PreToolUse hook (`pre-tool-use-compliance.sh`) calls `scripts/compliance_check.py` (Python regex + rule engine) for its classification decision — this is the correct architecture for a blocking gate. compliance-qa is the *nuanced second-pass* that draft-writer calls before writing, not a hook escalation target. The spec's intent (nuanced LLM check for hedged claims) is preserved; the invocation path is corrected. | Active — bridge not built because it cannot be built with the current hook model |

| 2026-04-22 | **Compliance gate enforces source existence + expiry + tier-4 exclusion; `approved_for_claims[]` enforcement is at validator time only.** `scripts/compliance_check.py` checks three things at runtime: (1) claim detected by regex has a citation in the draft frontmatter, (2) cited source exists in registry and is not expired, (3) cited source is not authority tier 4. It does NOT read `approved_for_claims[]`. Validator-time enforcement (`python3 -m scripts.validate_drafts`) checks `approved_for_claims[]` alignment for any `ClaimCited` entry where `claim_type` is declared. Runtime hook enforcement (promoting `ClaimCited.claim_type` to required + extending `_validate_source()` to check `approved_for_claims[]`) is targeted for 2026-04-23 as a schema-change milestone. Until then, brief-author and draft-writer subagents are instructed to respect `approved_for_claims[]` at authoring time, and `validate_drafts` enforces it at CI time when `claim_type` is present. README "Compliance model" section accurately reflects this two-tier state. | **CLOSED 2026-04-23** — see next row |
| 2026-04-23 | **Schema 1.1: `ClaimCited.claim_type` promoted to required; `approved_for_claims[]` enforcement added to runtime compliance gate.** `ClaimCited.claim_type` changed from `Optional[ClaimType]` to `ClaimType` in `schemas.py`. `_validate_source()` in `compliance_check.py` now checks that the detected claim's `claim_type` appears in the cited source's `approved_for_claims[]`. All schema_version literals bumped from `"1.0"` to `"1.1"` across all entity types (Brief, Draft, Source, AuditEvent, SubagentResponse). Migration stub at `scripts/migrations/draft_1_0_to_1_1.py` — no-op for production data (all ClaimCited entries already had claim_type set). Dual-read not required: no 1.0 drafts existed without claim_type at migration time. | Active |
